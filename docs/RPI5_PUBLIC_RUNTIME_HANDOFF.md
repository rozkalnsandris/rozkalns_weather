...
- Issue #462 added the source-only zero-input installer bridge for the exact 23-artifact operator closure while leaving host installation disabled.

These are source interfaces, not deployment proof. At the 2026-09-11 Issue #30 build snapshot, `RPi5_main/main=2672451d1f3ddc6cffcc70a3627c6b758db8abe0` and exact-main `Validate` run `34571359393` succeeded. The Composite operator remains source-ready/host-inactive and the #462 installer remains source-only/install-disabled; source merge proves neither host installation nor deployment. This weather repo does not mutate or activate `RPi5_main`.

`deploy/rpi5-source-binding.json` records that point-in-time source reconciliation. It also records a deliberate current blocker: `ops-workflows#46` still binds Weather candidate `52d3fd0ff946d3d5a0e1379f6c4362bf834a1aa9`, while Issue #30 was built from Weather `7b188d56ef083f60289bf34297199a42f13e1048`. Queue READY remains eligibility-only; the mismatch must block rather than be silently rewritten or bypassed.

`deploy/first-public-rollout-preflight.json` + `rozkalns-weather rollout-live-preflight-validate` define the JIT machine gate. They require fresh exact source/CI/queue/host/target/operator-install/baseline/auth/budget evidence and return `PASS` or explicit `BLOCKED` reasons without creating/consuming LIVE authorization. Before the Composite rollout can become PASS, operator installation must first be separately owner-authorized and verified, and the queue must independently match the final merged Weather SHA.

A future exact Composite LIVE envelope must freshly bind the trusted host and target alias, exact merged weather SHA and exact-SHA CI, current `RPi5_main` SHA + CI, matching READY queue, verified operator installation, sanitized current runtime baseline, exact reviewed identities, bounded dates/models/run hours and WMO `10416`, explicit recovery decision, exact release/supplemental/read-only budgets, authorization owner/TTL/raw-body/replay evidence and postconditions. It must not accept generic shell/path/argv/environment authority from GitHub text.
