...
- `RPi5_main` Issue #454 / PR #460 is merged at the 2026-09-10 reconciliation snapshot `RPi5_main/main=6711ef153ae6b8636ea0c77fd4224ea2dd3d8d3a`; exact-main `Validate` run `34514438900` succeeded. It adds the source-ready Composite operator with issue-number-only caller authority, durable consume-before-first-mutation semantics, JIT revalidation and the fixed one-shot stage launcher.
- `deploy/rpi5-source-binding.json` binds that source snapshot to `ops-workflows#46` eligibility for Weather candidate `52d3fd0ff946d3d5a0e1379f6c4362bf834a1aa9` without treating the queue's older embedded RPi5 snapshot as current authority.
