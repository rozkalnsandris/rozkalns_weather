    ...
    *,
    ...
    with _readonly_connection(database) as connection:
        runs = connection.execute(
            ...
            (BENCHMARK_LOCATION.id, *models, start_iso, end_iso),
        ).fetchall()
        ...
        forecast_valid = {
            ...
            for row in connection.execute(
                ...
                (BENCHMARK_LOCATION.id, *models, start_iso, end_iso, start_iso, end_iso),
            ).fetchall()
        }
        observed_valid = {
            str(row[0])
            for row in connection.execute(
                ...
                (BENCHMARK_TRUTH_STATION_ID, BENCHMARK_LOCATION.id, start_iso, end_iso),
            ).fetchall()
        }
        ...
        historical_ifs = connection.execute(
            ...
            (BENCHMARK_LOCATION.id, ifs_history_start, utc_iso(datetime.combine(COMMON_BENCHMARK_START, time.min, tzinfo=timezone.utc))),
        ).fetchone()
