...
def validate_source_package(root: Path | None = None) -> dict[str, object]:
    ...
    _require(production_bootstrap.get("contract") == "rozkalns-weather.production-public-corpus-bootstrap.v2", "production bootstrap contract mismatch")
    _require(production_bootstrap.get("status") == "SOURCE_READY_REQUIRES_EXACT_LIVE_DATA_AUTHORITY", "production bootstrap status mismatch")
    pb_window = production_bootstrap.get("fixed_common_window")
    ...
    pb_checkpoint = production_bootstrap.get("checkpoint")
    ...
    for label, value in (("window", pb_window), ("forecast", pb_forecast), ("truth", pb_truth), ("schema", pb_schema), ("checkpoint", pb_checkpoint), ("recovery", pb_recovery), ("destructive", pb_destructive), ("authority", pb_authority)):
    ...
    assert isinstance(pb_schema, dict) and isinstance(pb_checkpoint, dict) and isinstance(pb_recovery, dict)
    assert isinstance(pb_destructive, dict) and isinstance(pb_authority, dict)
    _require(pb_window.get("start_date") == "2026-08-13" and pb_window.get("end_date") == "2026-08-26", "production bootstrap fixed window mismatch")
    _require(pb_window.get("inclusive_days") == 14 and pb_window.get("fixed_non_rolling") is True, "production bootstrap fixed window bounds mismatch")
    _require(pb_window.get("checkpoint_namespace") == "fixed-window-20260813-20260826-v1" and pb_window.get("pre_159_fingerprint_reusable") is False, "production bootstrap fixed window identity mismatch")
    ...
    _require(pb_forecast.get("exact_run_transport_status") == "VERIFIED_COMPLETE_FIXED_WINDOW", "production bootstrap exact-run transport status mismatch")
    _require(pb_forecast.get("exact_init_required") is True and pb_forecast.get("full_horizon_required") is True, "production bootstrap exact-run provenance requirements mismatch")
    _require(pb_forecast.get("skip_unavailable_run_allowed") is False and pb_forecast.get("synthetic_or_imputed_run_allowed") is False, "production bootstrap forecast integrity weakened")
    ...
    _require(pb_truth.get("station_id") == "05480" and pb_truth.get("location_id") == "station_05480", "production bootstrap truth benchmark mismatch")
    _require(pb_truth.get("selected_common_window_start") == "2026-08-13" and pb_truth.get("selected_common_window_end") == "2026-08-26", "production bootstrap truth fixed window mismatch")
    ...
    _require(pb_checkpoint.get("namespace") == "fixed-window-20260813-20260826-v1" and pb_checkpoint.get("skip_ahead_allowed") is False, "production bootstrap checkpoint namespace mismatch")
    ...
    _require(systemd.get("service_unit") == "rozkalns-weather-public-ingest.service", "systemd service unit identity mismatch")
