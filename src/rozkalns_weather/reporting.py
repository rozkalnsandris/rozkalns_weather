...
def monthly_weather_next_report(database: Database, *, month: str) -> dict[str, Any]:
    ...
    with database.connect() as connection:
        rows = [dict(row) for row in connection.execute(
            """WITH forecast AS (
                SELECT r.provider,r.model_version,r.location_id,r.init_time_quality,
                       r.init_time_utc,v.valid_time_utc,v.lead_hours,
                       v.value AS forecast_value,
                       MAX(CASE WHEN v.statistic='p10' THEN v.value END)
                         OVER (PARTITION BY r.id,v.valid_time_utc,v.variable) AS p10,
                       MAX(CASE WHEN v.statistic='p90' THEN v.value END)
                         OVER (PARTITION BY r.id,v.valid_time_utc,v.variable) AS p90,
                       v.statistic
                FROM forecast_runs r JOIN forecast_values v ON v.run_id=r.id
                WHERE r.location_id=? AND v.variable='temperature_2m'
                  AND v.statistic IN ('deterministic','mean','p10','p90')
                  AND v.valid_time_utc>=? AND v.valid_time_utc<?
            )
            SELECT f.*,o.value AS observed_value
            FROM forecast f JOIN observations o
              ON o.variable='temperature_2m'
             AND o.observed_at_utc=f.valid_time_utc
             AND o.location_id=f.location_id
             AND o.source_provider='DWD'
            WHERE f.statistic IN ('deterministic','mean')
            ORDER BY f.valid_time_utc,f.provider""",
            (DWD_10416.id, start, end),
        ).fetchall()]
        release_events = [dict(row) for row in connection.execute(
    ...
    for row in rows:
        rows_by_bucket[lead_bucket(float(row["lead_hours"]))].append(row)
    ...
    for bucket, bucket_rows in sorted(rows_by_bucket.items()):
        ...
        for times in provider_times.values():
            common_times = set(times) if common_times is None else common_times & times
        ...
        for row in bucket_rows:
            if common_times and str(row["valid_time_utc"]) not in common_times:
                continue
            pair = ErrorPair(
            ...
            grouped[(pair.provider, pair.model_version or "unknown")].append(pair)
            if pair.provider == "weathernext3":
                notable.append({
                    "valid_time_utc": row["valid_time_utc"],
                    "lead_bucket": bucket,
                    "error_abs_degC": abs(pair.forecast - pair.observed),
                    "model_version": pair.model_version,
                })
        for (provider, version), items in sorted(grouped.items()):
            metrics.append({"provider": provider, "model_version": version, "lead_bucket": bucket, **summarize(items)})

    misses = sorted(notable, key=lambda item: float(item["error_abs_degC"]), reverse=True)[:10]
    wins = sorted(notable, key=lambda item: float(item["error_abs_degC"]))[:10]
    wn_n = sum(int(item["n"]) for item in metrics if item["provider"] == "weathernext3")
    return {
        "report_type": "weathernext_station_skill_monthly_v2",
        "month": month,
        ...
        "metrics": metrics,
        "notable_weather_next_misses": misses,
        "notable_weather_next_wins": wins,
        "release_notes_source": WEATHERNEXT_RELEASE_NOTES_URL,
        "release_note_events": release_events,
        "note": "Home forecasts are excluded from measured skill until a home observation source exists. Rankings use common valid timestamps within each lead bucket. Release events are included only when previously recorded from verified source metadata; report generation never invents them.",
    }
