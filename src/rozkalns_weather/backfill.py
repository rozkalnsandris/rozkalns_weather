...
def _require_ready_station_database(database: Database) -> None:
    ...
    with database.connect() as connection:
        row = connection.execute("SELECT id FROM locations WHERE id=?", (BENCHMARK_LOCATION.id,)).fetchone()
    if row is None:
        raise RuntimeError(
            f"{BENCHMARK_LOCATION.id} location is missing; explicit benchmark-location registration must establish it before backfill"
        )
    ...
    def forecast_runs(
        ...
        *,
        ...
        for index, init_time in enumerate(planned):
            ...
            self.database.insert_forecast_run(run, location_id=BENCHMARK_LOCATION.id)
...
def main() -> None:
    ...
    forecast = sub.add_parser("forecast", help="backfill exact Open-Meteo Single Runs at the pinned CDC benchmark location")
    ...
    truth = sub.add_parser("truth", help="backfill pinned DWD CDC station 01303 observations from official CDC station ZIPs")
