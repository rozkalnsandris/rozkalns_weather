...
def _daily_payload(database: Database, *, days: int, timezone_name: str) -> list[dict[str, object]]:
    ...
    tz = ZoneInfo(timezone_name); grouped: dict[tuple[str, str], dict[str, object]] = {}
    for row in rows:
        if row["statistic"] not in {"deterministic", "mean", "p50"}: continue
        local_date = parse_time(str(row["valid_time_utc"])).astimezone(tz).date().isoformat(); key = (str(row["provider"]), local_date)
        ...
        if row["variable"] == "temperature_2m":
            value = float(row["value"]); mn = item["temperature_min_c"]; mx = item["temperature_max_c"]; item["temperature_min_c"] = value if mn is None else min(float(mn), value); item["temperature_max_c"] = value if mx is None else max(float(mx), value)
        elif row["variable"] == "precipitation_1h" and row["accumulation_window_minutes"] == 60: item["precipitation_total_mm"] = float(item["precipitation_total_mm"]) + float(row["value"])
    return sorted(grouped.values(), key=lambda x: (str(x["date"]), str(x["provider"])))

def create_app(settings: Settings | None = None, database: Database | None = None) -> FastAPI:
    settings = settings or Settings.from_env(); database = database or Database(settings.database_url); database.initialize()
    ...
    app = FastAPI(title="rozkalns_weather", version="0.3.0", description="Private local weather comparison and WeatherNext 3 verification API"); app.state.settings = settings; app.state.database = database; static_dir = Path(__file__).with_name("static"); app.mount("/static", StaticFiles(directory=static_dir), name="static")
    @app.get("/")
    def index() -> FileResponse: return FileResponse(static_dir / "index.html")
    @app.get("/health")
    def health() -> dict[str, object]: return {"status": "ok", "database": "ready", "home_configured": settings.home_configured}
    @app.get("/api/providers")
    def providers() -> dict[str, object]: return {"providers": [_descriptor(provider) for provider in PROVIDERS]}
    @app.get("/api/health/providers")
    def provider_health() -> dict[str, object]:
        stored = database.provider_statuses(); now = datetime.now(timezone.utc); provider_states = []
        for provider in PROVIDERS:
            saved = stored.get(provider.id, {}); state = saved.get("state", "adapter_ready_not_ingested")
            if provider.id == "weathernext3" and provider.id not in stored: state = access_state(configured=settings.weathernext_cloud_configured, now=now)
            provider_states.append({"id": provider.id, "model_name": provider.model_name, "state": state, "last_success_at_utc": saved.get("last_success_at_utc"), "last_attempt_at_utc": saved.get("last_attempt_at_utc"), "last_init_time_utc": saved.get("last_init_time_utc"), "detail": saved.get("detail")})
    ...
    def corpus_stats() -> dict[str, object]: return database.corpus_stats()
    @app.get("/api/corpus/integrity")
    def corpus_integrity() -> dict[str, object]: return database.corpus_integrity()
    @app.get("/api/verification/summary")
    def verification_summary(days: int = Query(90, ge=1, le=3650)) -> dict[str, object]:
        rows = database.temperature_verification_pairs(days=days, location_id=DWD_10416.id); by_provider = defaultdict(list); by_bucket = defaultdict(lambda: defaultdict(list)); by_version = defaultdict(lambda: defaultdict(list))
        for row in rows:
            ...
            by_provider[pair.provider].append(pair); by_bucket[pair.provider][lead_bucket(pair.lead_hours)].append(pair); by_version[pair.provider][pair.model_version or "unknown"].append(pair)
    ...
    def precipitation_verification(days: int = Query(90, ge=1, le=3650)) -> dict[str, object]:
        threshold = settings.precipitation_event_threshold_mm; rows = database.precipitation_verification_pairs(days=days, threshold_mm=threshold, location_id=DWD_10416.id); prob = defaultdict(list); amount = defaultdict(list)
        for row in rows["probability"]: prob[str(row["provider"])].append(ProbabilityPair(provider=str(row["provider"]), model_version=row.get("model_version"), lead_hours=float(row["lead_hours"]), probability=float(row["probability"]), observed_event=float(row["observed_event"])))
        for row in rows["amount"]: amount[str(row["provider"])].append(ErrorPair(provider=str(row["provider"]), model_version=row.get("model_version"), lead_hours=float(row["lead_hours"]), forecast=float(row["forecast_value"]), observed=float(row["observed_value"])))
    ...
    def warnings() -> dict[str, object]:
        if not settings.home_configured: raise HTTPException(status_code=503, detail="home location is not configured")
