...
from rozkalns_weather.providers.open_meteo import (
    ...
    decode_open_meteo_json_response,
...
def test_model_keys_and_metadata_domains_are_explicit() -> None:
    ...
    assert ICON_D2.meta_domain == "dwd_icon_d2"
...
def test_single_run_request_has_exact_run_and_no_probability_field() -> None:
    ...
    adapter = OpenMeteoSingleRunAdapter(ICON_D2, fetcher=fetcher)
    init = datetime(2026, 9, 7, 6, tzinfo=timezone.utc)
    available = datetime(2026, 9, 7, 8, tzinfo=timezone.utc)
    run = adapter.fetch(
        lat=51.5,
        lon=7.6,
        init_time=init,
        availability_time=available,
        retrieved_at=datetime(2026, 9, 7, 8, 30, tzinfo=timezone.utc),
    )
    params = seen[0][1]
...
def test_http_200_non_json_model_run_unavailable_has_stable_reason_code() -> None:
    body = (
        ...
        "modelRunUnavailable(model: App.DomainRegistry.dwd_icon_d2_15min, run: example)"
    )
    ...
    assert caught.value.content_type == "application/json; charset=utf-8"


def test_http_400_json_model_run_unavailable_has_same_reason_code() -> None:
    with pytest.raises(OpenMeteoTransportError) as caught:
        decode_open_meteo_json_response(
            status_code=400,
            content_type="application/json",
            body='{"error":true,"reason":"The requested model run is not available. Model: dwd_icon_d2"}',
        )
    assert caught.value.reason_code == OPEN_METEO_MODEL_RUN_UNAVAILABLE
    assert caught.value.status_code == 400


def test_unrelated_non_json_response_is_not_silently_relabelled_as_missing_run() -> None:
    with pytest.raises(OpenMeteoTransportError) as caught:
        decode_open_meteo_json_response(
            status_code=200,
            content_type="text/plain",
            body="upstream proxy returned malformed data",
        )
    assert caught.value.reason_code == OPEN_METEO_NON_JSON_RESPONSE
    assert caught.value.reason_code != OPEN_METEO_MODEL_RUN_UNAVAILABLE
