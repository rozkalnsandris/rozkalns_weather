...
def _complete_evidence() -> dict[str, object]:
    ...
    def model_state() -> dict[str, object]:
        return {
            "location_id": "station_dwd_cdc_01303",
    ...
    return {
        ...
        "benchmark_location": {"location_id": "station_dwd_cdc_01303", "registered": True},
        "truth": {
            "station_id": "01303",
            "location_id": "station_dwd_cdc_01303",
...
def test_plan_freezes_bounds_scope_colocation_and_verified_truth_transport() -> None:
    plan = _plan()
    ...
    assert plan["truth_transport"] == {
        ...
        "station_id": "01303",
        "location_id": "station_dwd_cdc_01303",
        "historical_capability": "verified_frozen_window_capability",
    ...
    assert plan["forecast_truth_colocation"]["location_id"] == "station_dwd_cdc_01303"
    assert plan["benchmark_location_registration_required"] is True
...
def test_complete_evidence_passes_source_contract_but_never_grants_authority() -> None:
...
def test_partial_duplicate_revision_location_and_interrupted_states_fail_closed() -> None:
    plan = _plan()
    ...
    wrong_location = _complete_evidence()
    wrong_location["forecasts"]["icon_d2"]["location_id"] = "station_10416"
    assert "ICON_D2_LOCATION_MISMATCH" in evaluate_resume_evidence(plan, wrong_location)["block_reasons"]
...
def test_resume_evidence_requires_explicit_benchmark_registration() -> None:
    evidence = _complete_evidence()
    evidence["benchmark_location"]["registered"] = False
    assert "BENCHMARK_LOCATION_NOT_REGISTERED" in evaluate_resume_evidence(_plan(), evidence)["block_reasons"]
...
def test_production_bootstrap_plan_cli_is_runtime_independent_and_requires_explicit_live_authority(monkeypatch, capsys) -> None:
    monkeypatch.setattr(
        "sys.argv",
        [
            "rozkalns-weather",
    ...
    assert payload["state"] == "READY_FOR_EXPLICIT_LIVE_DATA_AUTHORIZATION"
    assert payload["block_reasons"] == []
    assert payload["identity"]["truth_station_id"] == "01303"
