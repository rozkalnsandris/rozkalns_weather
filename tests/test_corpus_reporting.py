...
def test_complete_common_day_passes_and_is_read_only(tmp_path: Path) -> None:
    ...
    assert report["common_window"]["location_id"] == BENCHMARK_LOCATION.id
    assert report["observations"]["station_id"] == BENCHMARK_TRUTH_STATION_ID
    ...
    assert all(all(bucket["present_runs"] == 4 for bucket in item["lead_bucket_coverage"].values()) for item in report["models"])
    assert before == after
...
def test_revision_drift_blocks_without_rewriting_history(tmp_path: Path) -> None:
    ...
    database.insert_forecast_run(_run(ECMWF_IFS, init, value=12.0, retrieved_at=init + timedelta(minutes=1)), location_id=BENCHMARK_LOCATION.id)
    report = public_corpus_report(database, start=date(2026, 4, 2), end=date(2026, 4, 2))
    assert report["state"] == "BLOCKED"
...
def test_historical_ifs_is_separate_from_common_readiness(tmp_path: Path) -> None:
    ...
    database.insert_forecast_run(_run(ECMWF_IFS, historical), location_id=BENCHMARK_LOCATION.id)
    report = public_corpus_report(database, start=date(2026, 4, 2), end=date(2026, 4, 2))
    assert report["state"] == "PASS"
...
def test_cli_corpus_report_emits_machine_readable_pass(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["rozkalns-weather", "corpus-report", "--start", "2026-04-02", "--end", "2026-04-02"])
    cli_main()
    payload = json.loads(capsys.readouterr().out)
    assert payload["state"] == "PASS"
    assert payload["read_only"] is True
    assert payload["common_window"]["location_id"] == BENCHMARK_LOCATION.id
